    ...
    socket.ev.on('connection.update', async (update) => {
      ...
      if (connection === 'open') {
        try {
          ...
          // ── PER-BOT SETTINGS: set the JID so DB operations are isolated ──
          const cleanBotJid = sanitizedNumber;   // e.g. "94781332957"
          config.BOT_JID = cleanBotJid;
          try {
            const { initSettingsForBot, updb: updbFn } = require('./lib/database');
            await initSettingsForBot(cleanBotJid);
            await updbFn();
            console.log(`✅ Per-bot settings loaded for ${cleanBotJid}`);
          } catch (dbErr) {
            console.error('Per-bot DB init error:', dbErr);
          }
          // ─────────────────────────────────────────────────────────────────
